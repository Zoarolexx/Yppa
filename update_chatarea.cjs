...
export function ChatArea({ user, currentUser, messages, onSendMessage, onBack }: ChatAreaProps) {
  ...
  const handleReportUser = async () => {
    ...
    if (reason) {
       await adminFunctions.createReport(currentUser.id, user.id, reason, "User reported from chat");
       alert("User reported!");
    }
    setShowMenu(false);
  };
  ...
  if (selectedMsg && isMeSelected && selectedMsg.type === 'text' && selectedMsg.text !== 'Pesan telah dihapus') {
     const createdAt = selectedMsg.originalCreatedAt?.seconds * 1000;
     const now = Date.now();
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#E5DDD5] relative" style={{ backgroundImage: 'url("https://web.telegram.org/a/chat-bg-pattern-light.png")', backgroundSize: '400px', backgroundRepeat: 'repeat', backgroundColor: '#e4eef6', backgroundBlendMode: 'overlay' }}>
      ...
      {selectedMessageId ? (
        ...
        <div className="h-[60px] bg-white flex items-center justify-between px-2 sm:px-4 border-b border-gray-200 z-10 shadow-sm relative">
          <div className="flex items-center gap-2 sm:gap-3 cursor-pointer hover:bg-gray-50 px-1 sm:px-2 py-1 rounded-lg transition-colors flex-1 min-w-0">
            {onBack && (
            ...
            <div className="relative">
              ...
              {showMenu && (
                <div className="absolute right-0 top-full mt-1 w-48 bg-white rounded-lg shadow-lg border border-gray-100 py-1 z-50">
                  <button className="w-full text-left px-4 py-2 hover:bg-gray-50 flex items-center gap-2 text-sm text-gray-700" onClick={() => { setShowMenu(false); alert("Blocked!"); }}>
            ...
            <div key={message.id} 
              ...
              <div
                className={\`max-w-[70%] rounded-2xl px-3 py-2 shadow-sm relative group \${
                  isMe ? (isSelected ? 'bg-[#d8eed3] rounded-br-none' : 'bg-[#EEFFDE] rounded-br-none') : (isSelected ? 'bg-blue-50 rounded-bl-none' : 'bg-white rounded-bl-none')
                }\`}
              >
